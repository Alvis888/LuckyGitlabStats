...
namespace LuckyGitlabStatWebAPI.Controllers
{
    public class RootController : ApiController
    {
        ...
        public int GetProjectAdd_Weekly_Monthly_Yearly(int queryDays)
        {
            ...
            if (queryDays == 7)
            {
                querySingleInfo.CommandText = " select top 2 DATEPART(wk,committime) as month, count(DATEPART(wk, committime)) as times from Memberproject where DATEPART(wk, committime) >= (DATEPART(m, getdate()) - 2) and DATEPART(yy, committime) = (DATEPART(yy, getdate()))  group by DATEPART(wk, committime)   order by DATEPART(wk, committime) desc";
            ...
            {
                querySingleInfo.CommandText = "  select top 2 DATEPART(yy,committime) as month, count(DATEPART(yy, committime)) as times from Memberproject where DATEPART(yy, committime) >= (DATEPART(yy, getdate()) - 2) group by DATEPART(yy, committime) order by DATEPART(yy, committime) desc";
                SqlDataReader getTotalCommitTimesReader = querySingleInfo.ExecuteReader();
                //有多行数据，用while循环
                while (getTotalCommitTimesReader.Read())
                {
        ...
        public int GetGroupAdd_Weekly_Monthly_Yearly(int queryDays)
        {
            ...
            if (queryDays == 7)
            {
                querySingleInfo.CommandText = " select top 2 DATEPART(wk, buildtime) as month,	count(DATEPART(wk, buildtime)) as times from Membergroup where DATEPART(wk, buildtime) >= (DATEPART(wk, getdate()) - 2)   and DATEPART(yy, buildtime) = (DATEPART(yy, getdate())) group by DATEPART(wk, buildtime) order by DATEPART(wk, buildtime) desc ";
                SqlDataReader getTotalCommitTimesReader = querySingleInfo.ExecuteReader();
                //有多行数据，用while循环
                while (getTotalCommitTimesReader.Read())
                {
            ...
            if (queryDays == 30)
            {
                querySingleInfo.CommandText = " select top 2  DATEPART(m, buildtime) as month, 	count(DATEPART(m, buildtime)) as times from Membergroup where DATEPART(m, buildtime) >= (DATEPART(m, getdate()) - 2)    and DATEPART(yy, buildtime) = (DATEPART(yy, getdate())) group by DATEPART(m, buildtime) order by DATEPART(m, buildtime) desc";
                SqlDataReader getTotalCommitTimesReader = querySingleInfo.ExecuteReader();
                //有多行数据，用while循环
                while (getTotalCommitTimesReader.Read())
                {
        ...
        public int GetUserAdd_Weekly_Monthly_Yearly(int queryDays)
        {
            ...
            if (queryDays == 7)
            {
                querySingleInfo.CommandText = " select top 2 DATEPART(wk, buildtime) as weekly,	count(DATEPART(wk, buildtime)) as times from Member where DATEPART(wk, buildtime) >= (DATEPART(wk, getdate()) - 2)   and DATEPART(yy, buildtime) = (DATEPART(yy, getdate())) group by DATEPART(wk, buildtime) order by DATEPART(wk, buildtime) desc ";
                SqlDataReader getTotalCommitTimesReader = querySingleInfo.ExecuteReader();
                //有多行数据，用while循环
                while (getTotalCommitTimesReader.Read())
                {
